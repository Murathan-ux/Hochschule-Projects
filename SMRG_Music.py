...
class YouTubeMoodPlayer:
            ...
            if self.player and self.playing:
                try:
                    ...
                    if pos >= 0:
                        ...
                        remaining = max(self.duration - pos, 0)
                        self.remaining_label.config(text=self.format_time(remaining))
                except:
                    pass
            time.sleep(1)
    ...
    def seek_song(self, value):
        if self.player and self.duration > 0:
            seek_time = (float(value) / 100) * self.duration * 1000
    ...
    def toggle_voice_detection(self):
        if self.voice_detecting:
            ...
            self.status_label.config(text="Listening for mood...")
    ...
    def voice_detection_thread(self):
        ...
        while self.voice_detecting:
            try:
                ...
                mood = None
                for key in playlist_links.keys():
                    if key in text:
                        mood = key
                        break

                if mood and mood != detected_mood:
                    detected_mood = mood
